...
public class EmployeeController {
    ...
    public ResponseEntity<List<Employee>> findAllByDepartmentId(@PathVariable Long id) throws InterruptedException {
        return ResponseEntity.ok(employeeService.findByDepartmentId(id));
    }

    @PostMapping
    public ResponseEntity<Employee> create(@RequestBody Employee employee){
        return employeeService.create(employee).map( (e)->
    ...
    public ResponseEntity<?> delete(@PathVariable Long id){
        return employeeService.delete(id).map( (e)->ResponseEntity.noContent().build() )
                .orElse(ResponseEntity.notFound().build());
